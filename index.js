...
async function onPageLoad() {
  ...
  if (queryParams.has('code')) {
    ...
    console.log("checking user",thisUser,thedata.Token)
    if (theDatanotNull){
...
document.addEventListener("DOMContentLoaded", async() => {
    ...
    if(linkedinButton){
      ...
      window.location.href = "https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=863iuogbciqxja&redirect_uri=https://multiagentbase-pro-93abd0.webflow.io/playground?platform=app1&state=foobar&scope=w_member_social,openid,profile,email"
      })
    }
    
    if(calendarButton){
      document.querySelector("#calendaroauth").addEventListener("click", async (event) =>{
      window.location.href = "https://accounts.google.com/o/oauth2/v2/auth?client_id=142417176441-2f2g7mqv6fg7sd8avi9agqvt2comleg2.apps.googleusercontent.com&redirect_uri=https://multiagentbase-pro-93abd0.webflow.io/playground?platform=app2&response_type=code&scope=https://www.googleapis.com/auth/calendar https://www.googleapis.com/auth/calendar.events https://www.googleapis.com/auth/calendar.events https://www.googleapis.com/auth/calendar.events.readonly https://www.googleapis.com/auth/calendar.settings.readonly https://www.googleapis.com/auth/calendar.addons.execute&access_type=offline"
      })
    }

    if(xoauthButton){
      document.querySelector("#Xoauth").addEventListener("click", async (event) =>{
      window.location.href = "https://twitter.com/i/oauth2/authorize?response_type=code&client_id=TVQyTzFpLUF0dGttU1pyaVVFZ2s6MTpjaQ&redirect_uri=https://multiagentbase-pro-93abd0.webflow.io/playground?platform=app3&scope=tweet.read%20users.read%20offline.access%20tweet.write%20follows.read%20follows.write&state=x&code_challenge=x56fdfdfgh&code_challenge_method=plain"
      })
    }
...
async function authenticate(code,platform){
  ...
  if (user) {
    ...
    itemsCollection.onSnapshot((snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        let alreadyAdded=!appsId.includes(thisId)
        let appNotEmpty=doc.data().app!=undefined&&doc.data().app!=null
        let tokenNotEmpty=doc.data().token!=undefined&&doc.data().token!=null
        let theapp1=doc.data().app
        
        let thetoken2=doc.data().token
        console.log("testapp",theapp1,thetoken2,doc.data())
